...
func action_process(_delta: float) -> void:
	...
	## Input
	if not Input.is_action_just_pressed("Use Grappling Hook"):
		return
	if performing:
...
func apply_grappling_hook_swing():
	...
	#region point on circle and tangents, old stuff
	## The point equal to an intersection between a circle drawn by the
	## rope 
	var last_attachment_pos: Vector2 = v3_to_v2(last_attachment.global_position)
	var _nearest_point_on_circle: Vector2 = \
	last_attachment_pos.direction_to(
		v3_to_v2(player.global_position) * \
		last_attachment.rope_lenght + \
		last_attachment_pos
		)

	#var tangent_left: Vector3 = \
	#(last_attachment.to_local(
		#nearest_point_on_circle)).cross(Vector3.LEFT).normalized()
	#var tangent_right: Vector3 = \
	#(last_attachment.to_local(
		#nearest_point_on_circle)).cross(Vector3.RIGHT).normalized()
	#endregion
